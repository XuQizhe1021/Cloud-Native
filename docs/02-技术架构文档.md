...
老师 PDF 明确要求先完成聚合项目创建，因此后端建议采用如下结构：

1. 父工程 `backend-parent`
2. 父工程 `packaging` 设为 `pom`
3. 在父工程统一管理 Spring Boot、Spring Cloud、数据库驱动、测试依赖版本
4. 子模块至少包含：
   - `eureka-server`
   - `gateway-service`
   - `user-service`
...
这样既符合课程实践顺序，也方便后续 Jenkins 统一构建。
...
| 实践阶段 | PDF 核心要求 | 本项目对应实现 |
|---|---|---|
| 实践一 | 聚合项目、Eureka、自身注册、数据库创建 | 父工程 + `eureka-server` + 3 个业务库 schema |
| 实践二 | Feign 服务调用、负载均衡、服务注册发现、数据库及表 | `repair-service` 调 `user-service` / `inventory-service`，库存服务多副本 |
| 实践三 | OpenFeign 熔断、Gateway API 网关、访问权限验证 | Resilience4j fallback + Gateway JWT 鉴权与统一路由 |
...
结合老师 PDF，“实现每个微服务所用的数据库及表”应理解为：

1. `user-service` 至少有 `users` 等核心用户表
2. `repair-service` 至少有 `repair_order`、`repair_part_usage` 等工单相关表
3. `inventory-service` 至少有 `parts` 等库存表
4. 答辩时不仅要展示接口，还要能说明表结构与服务职责的对应关系
...
## 10.3 课程验证场景建议
1. 展示 `eureka-server` 中各服务实例均已注册
2. 让 `inventory-service` 运行 2 个副本，演示 Feign + LoadBalancer 的调用效果
3. 人为关闭一个库存服务实例或制造超时，验证熔断降级提示是否生效
4. 通过未登录请求访问受保护接口，验证 Gateway 权限校验是否拦截成功
